...
class QuantumImprovementTester:
    ...
    def save_results(self, filepath):
        """Save test results to a JSON file (path or str)."""
        results_data = []
        for result in self.results:
        ...
        logger.info(f"Test results saved to {fp}")
...
def run_terminal_tests():
    ...
    # Save results under results/test_results/
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    out_dir = _ROOT / "results" / "test_results"
