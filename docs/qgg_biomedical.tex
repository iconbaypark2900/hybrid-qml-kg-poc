% One-column A4 article (same content as QCE26 IEEE short paper)
% Build from docs/: pdflatex qgg_biomedical && bibtex qgg_biomedical && pdflatex qgg_biomedical && pdflatex qgg_biomedical
% (BST/search path: this directory contains IEEEtran.bst and references_qce26.bib.)
%
% Prior QCE26 IEEEtran build (reference):
%   TEXINPUTS=./qce26-template//: pdflatex paper_qGG_qce26 && ...
%
% EasyChair / submission metadata (IEEE Quantum Week 2026 topics:
%   https://qce.quantum.ieee.org/2026/conference-topics/ )
%   Primary subject area:   Quantum Machine Learning (QML)
%   Track:                  New Ideas and Emergent Results (Short Paper)
%   Secondary subject area: Quantum Applications
%     (biomedical / medicine & precision health; aligns with drug repurposing)
%   Keywords (paste into form if comma-separated):
%     quantum kernel methods, knowledge graph embeddings, link prediction,
%     drug repurposing, quantum support vector machine, Hetionet,
%     stacking ensemble, mechanism-of-action features
...
\item \textbf{Counterintuitive quantum-classical synergy.} Pauli feature map
  degrades standalone QSVC ($-8.7$ pp) yet raises ensemble PR-AUC ($+5.8$ pp)
  via classical-quantum error decorrelation.
...
No Nystr\"{o}m approximation \cite{williams2001nystrom,qiskit_ml2023} is used
(\texttt{nystrom\_m=None}); the full $1{,}208\!\times\!1{,}208$ kernel matrix
required \textbf{2,619 s}---a genuine full-dataset computation.
...
\textbf{Reproducing the primary result (PR-AUC 0.7987):}
...
python scripts/run_optimized_pipeline.py
  ...
  --ensemble_method stacking --tune_classical
  --fast_mode
\end{verbatim}}
Results written to \texttt{results/optimized\_results\_}\linebreak\texttt{YYYYMMDD-HHMMSS.json}.
...
\begin{table}[t]
\caption{Primary short-paper result: RotatE-128D, Pauli-16Q QSVC, stacking.
\textit{Full-graph RotatE 128D (200 ep.), Pauli reps=2, PCA$\to$24D$\to$16Q,
$C=0.1$, hard negatives 1:1, tuned classical. Kernel time 2,619~s.}}
\label{tab:primary}
\centering\small
...
\midrule
\textbf{Ensemble-QC (Pauli)} & \textbf{0.7987} & 0.7456 & 0.576 \\
RandomForest-Opt & 0.7838 & 0.7319 & 0.583 \\
ExtraTrees-Opt & 0.7807 & 0.7301 & 0.662 \\
Ensemble-QC (ZZ)$^\dagger$ & 0.7408 & --- & 0.649 \\
QSVC (ZZ)$^\dagger$ & 0.7216 & --- & 0.656 \\
QSVC (Pauli) & 0.6343 & 0.6313 & 0.586 \\
\bottomrule
\multicolumn{4}{l}{\footnotesize $^\dagger$From result file 20260216-091710 (same split).}
\end{tabular}
\end{table}

Table~\ref{tab:primary} reports the primary short-paper result; the full pipeline producing these numbers is shown in Figure~\ref{fig:pipeline}. The stacking ensemble improves over best classical (RF, 0.7838) by
\textbf{+0.0149 PR-AUC} (+1.49 pp).

In an extended 256D RotatE/Optuna configuration, the ensemble reaches
PR-AUC 0.8581 while QSVC remains most useful as a diversity-inducing base
learner rather than a standalone replacement.
...
On Hetionet CtD, our system achieves PR-AUC 0.7987 (RotatE-128D, Pauli-16Q,
genuine 2,619 s kernel, +1.49 pp over best classical) and PR-AUC 0.8581
(RotatE-256D, Pauli-12Q, Optuna-tuned).
The central finding reframes quantum kernel selection: maximize
classical-quantum prediction \emph{decorrelation}, not standalone accuracy.
...
This work establishes a new research direction at the intersection of quantum
kernel methods and KG reasoning for biomedicine; next steps include multi-seed
validation, MoA late fusion, and multi-relation extension.
