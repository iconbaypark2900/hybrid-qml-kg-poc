...
% Blended v2 of paper_qGG_mjack_v041726.tex + paper_qGG_mjack_v041826.tex
...
\small Quantum Global Group, Miami, FL 33132\textsuperscript{\hspace{-0.05em}$\dagger$}%
}
...
\FloatBarrier
%% ==================================================================
\section{Hybrid quantum-classical pipeline for Compound-treats-Disease (CtD) link prediction on Hetionet}
%% ==================================================================
...
For a compound $c$ and a disease $d$ let $T(c) = \{g : (c, \mathrm{CbG}, g) \in E\}$ be the set of genes bound by $c$; $G(d) = \{g : (d, \mathrm{DaG}, g) \in E\}$ the set of genes associated with $d$; $P(g)$ the set of pathways a gene participates in (GpPW); $\mathrm{PC}(c)$ the set of pharmacologic classes of $c$ (PCiC); $S(c) = \{c' : (c, \mathrm{CrC}, c') \in E\}$ the set of chemically similar compounds; and $S(d)$ the analogous set for similar diseases. Let $\mathcal{T}_{\mathrm{train}}$ denote the set of (compound, disease) pairs with a known CtD edge in the training set. The ten features are defined in Table~\ref{tab:moa} with the signal they capture.
