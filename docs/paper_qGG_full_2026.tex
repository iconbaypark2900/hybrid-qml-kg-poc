...
Expected QSVC kernel computation time: approximately 43 minutes on CPU. Results are written to \texttt{results/optimized\_results\_YYYYMMDD-HHMMSS.json}. Complete CLI flag dumps for the primary and extended headline runs are stored alongside the JSON artifacts under \texttt{results/} (see Tables~\ref{tab:primary} and~\ref{tab:extended} for timestamps).
...
Table~\ref{tab:primary} reports the primary benchmark; the full pipeline producing these numbers is shown in Figure~\ref{fig:pipeline}. The stacking ensemble improves over the best classical model (RandomForest, 0.7838) by \textbf{+0.0149 PR-AUC} (+1.49 pp).
...
Table~\ref{tab:extended} reports the extended results. The 256D configuration demonstrates that embedding dimensionality is the dominant performance driver. Upgrading from 128D to 256D RotatE embeddings lifts the classical RF baseline from 0.7838 to 0.8569 (+7.3 pp absolute) and the ensemble ceiling from 0.7987 to 0.8581 (+5.9 pp). Although standalone QSVC performance remained comparable to classical baselines, its contribution within the stacked ensemble improved prediction diversity, supporting the hypothesis that quantum kernels capture complementary relational geometry not fully represented in classical embedding spaces. The Optuna-tuned QSVC C=0.676 (vs.\ manual C=0.1) also contributes to the QSVC improvement (0.6343 $\to$ 0.7222).
...
\item \textbf{Pauli--Ensemble:} \textbf{0.7987 PR-AUC} --- the headline number. Adding Pauli-QSVC into the same stacking ensemble raises ensemble PR-AUC by +5.8~pp over the ZZ ensemble and by +1.5~pp over the best classical baseline.
\end{itemize}

\noindent\textbf{The inversion.} Standalone QSVC accuracy and ensemble PR-AUC move in \emph{opposite} directions when switching ZZ$\to$Pauli. The naive ``pick the higher single-model bar'' heuristic would have selected ZZ and missed the headline result.
...
We have presented the first hybrid quantum-classical pipeline that bridges knowledge graph embeddings with quantum kernel classifiers for biomedical link prediction. On Hetionet CtD we obtain \textbf{PR-AUC 0.7987} (primary: RotatE-128D, Pauli-16Q, full kernel, +1.49 pp vs.\ best classical) and \textbf{0.8581} (extended: RotatE-256D, Pauli-12Q, Optuna-tuned classical).
