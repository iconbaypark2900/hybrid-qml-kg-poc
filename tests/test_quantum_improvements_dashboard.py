...
from tests.test_quantum_improvements_terminal import QuantumImprovementTester, TestStatus

def _test_results_dir() -> Path:
    return _ROOT / "results" / "test_results"


def load_test_results():
    """Load test results from JSON files."""
    results_dir = _test_results_dir()
    ...
    latest_file = max(
        json_files,
        key=lambda x: os.path.getctime(results_dir / x),
    )
    with open(results_dir / latest_file, "r") as f:
        return json.load(f)
...
def run_new_tests():
    ...
    if st.button("Run All Tests", type="primary"):
        with st.spinner("Running quantum improvements tests..."):
            ...
            out_dir = _test_results_dir()
            out_dir.mkdir(parents=True, exist_ok=True)
            filepath = out_dir / f"test_results_{timestamp}.json"
...
def show_system_health():
    ...
    if json_files:
        ...
        for file in recent_files:
            created_time = datetime.fromtimestamp(os.path.getctime(results_dir / file))
